...
export default {
  ...
  deleteFloor(floorId) {
    ...
    return new Promise((resolve) => {
      setTimeout(() => {
        const indexOffice = dataOffice.value.findIndex((office) => Number(officeId) === office.id);
        const floorIndex = dataOffice.value[indexOffice].floors.findIndex((floor) => floor.id === Number(floorId));
  ...
  updateFloor(data) {
    return new Promise((resolve) => {
      ...
      setTimeout(() => {
        ...
        const indexFloor = dataOffice.value[indexOffice].floors.findIndex((floor) => Number(floor.id) === Number(data.id));
        const newFloor = {
